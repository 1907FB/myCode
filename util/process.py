...
from util.myConv import student2dict, wei2dict
...
def validate(data_loader, model, criterion, epoch, monitors, args):
    ...
    for batch_idx, (inputs, targets) in enumerate(data_loader):
        with t.no_grad():
            ...
            targets = targets.cuda()
            outputs = model(inputs)
            if constant.valid_cal:
                with open('./result/result4.json', 'w', encoding='utf-8')as fp:
                    # 把python类型的数据以json格式储存到文件中
                    json.dump(constant.valid_cal, fp, ensure_ascii=False, default=wei2dict)
                print("INFO: save....")
                exit(0)
            loss = criterion(outputs, targets)
...
class PerformanceScoreboard:
    ...
    def update(self, top1, top5, epoch):
        if epoch > constant.epo_num[len(constant.epo_num)-1] and constant.flag:
            self.board.clear()
            constant.flag = False
